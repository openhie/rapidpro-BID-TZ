...
class PixSumbitNewBirthRegistration extends PixSumbit {
	...
	if((string)(int)$birth_details[0]==$birth_details[0]) {
            header('Content-Type: application/json');
            echo '{
                   "error":"Jina La Kwanza La Mama Halijajazwa",
                   "response":"Kuzaliwa Kwa '.$request["birth_details"].' Hakujasajiliwa,Rudia Tena"
                  }';
	...
	if((string)(int)$birth_details[1]==$birth_details[1]) {
           header('Content-Type: application/json');
            echo '{
                   "error":"Jina La Pili La Mama Halijajazwa",
                   "response":"Kuzaliwa Kwa '.$request["birth_details"].' Hakujasajiliwa,Rudia Tena"
                  }';
	...
	if((string)(int)$birth_details[4]==$birth_details[4]) {
		...
		if($year<1000) {
			echo '{
                   		"error":"Mwaka Ulioweka Sio Sahihi,Mwaka Unatakiwa Uwe Na Tarakimu 4",
                   		"response":"Kuzaliwa Kwa '.$request["birth_details"].' Hakujasajiliwa,Rudia Tena"
                  	      }';
	...
	$year=date("Y");
        $date=strtotime($year."-".$birth_details[3]."-".$birth_details[2]);
        $date=date("Y-m-d",$date);
        $today=date("Y-m-d");
	if($date>$today) {
                        echo '{
                                "error":"Tarehe Yakuzaliwa Si Sahihi",
                                "response":"Kuzaliwa Kwa '.$request["birth_details"].' Hakujasajiliwa,Rudia Tena"
                              }';
                        error_log("Wrong Year Format");
                        return false;

...
class PixSumbitNewBirthRegistration extends PixSumbit {
	...
	if (!is_array($request)
	    ...
	    header('Content-Type: application/json');
            echo '{
		   "error":"No Birth Details",
		   "response":"Birth Registration For '.$request["birth_details"].' Failed"
		  }';
	    return false;
	}
	$gender ='';
	...
	if (count($birth_details) < 4) {
	   header('Content-Type: application/json');
            echo '{
                   "error":"Not Enough Birth Details",
                   "response":"Birth Registration For '.$request["birth_details"].' Failed"
                  }';
	...
	if (! array_key_exists('orgid',$request) 
	   ...
	   error_log("No org id");
            echo '{
                   "error":"No Village Mapped To You",
                   "response":"Birth Registration For '.$request["birth_details"].' Failed"
                  }';
	    return false;
	}
